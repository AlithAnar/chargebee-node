                ...
                function(error, result) {
                    if (error) {
                        ...
                        if (time_machine.time_travel_status === 'succeeded') {
                            ...
                            setTimeout(retrieve, _self.env.timemachineWaitInMillis);
                        } else {
...
ChargeBee._core = (function() {
    var coreRef = {};
    var tc3jsc = require('timecamp-v3-javascript-client');
    ...
    coreRef.makeApiRequest = function(env, callBack, httpMethod, urlPrefix, urlSuffix, urlIdParam, params, headers, isListReq) {
        ...
        var http = new tc3jsc.BlipClient.HttpOverWebSocketAgent({ baseUrl: `${env.protocol}://${env.site}${env.hostSuffix}`, proxyUrl: env.wsProxyUrl, overwriteHeaders: {} });
        var protocol = http
        ...
        var req = protocol.request(path, {
          ...
          if(error) {
            console.error(String.fromCharCode.apply(String, error.data))
          } else {
            response.json().then(
